...
export default function LoginScreen() {
  ...
  const tick = useCallback(() => {
    ...
    if (bgRef.current)
      bgRef.current.style.transform = `translate3d(${x * -18}px, ${y * -12}px, 0) scale(1.14)`
    if (midRef.current)
      midRef.current.style.transform = `translate3d(${x * -32}px, ${y * -22}px, 0)`
    if (fgRef.current)
      fgRef.current.style.transform = `translate3d(${x * -5}px, ${y * -3}px, 0)`
  ...
  return (
    <div className="ls-root">
      {/* ── Parallax layer 1: imagem de fundo ── */}
      <div ref={bgRef} className="ls-layer ls-bg" />

      {/* ── Parallax layer 2: atmosfera de cor ── */}
      <div ref={midRef} className="ls-layer ls-mid" />

      {/* ── Layer 3: runas flutuantes ── */}
      <div className="ls-layer ls-particles">
        {PARTICLES.map(p => (
          <span
            key={p.id}
            className={`ls-rune ls-rune--${p.type}`}
      ...
      {/* ── Parallax layer 4: chão / profundidade ── */}
      ...
      <div className="ls-card">
        ...
        <form className="ls-form" onSubmit={handleSubmit} noValidate>
          ...
          <button type="submit" className={`ls-btn${loading ? ' ls-btn--loading' : ''}`} disabled={loading}>
            ...
            {loading
              ? 'VERIFICANDO FICHA...'
              : mode === 'login' ? 'ENTRAR NO SISTEMA' : 'CRIAR FICHA'}
          </button>

          <button type="button" className="ls-toggle" onClick={() => { setMode(m => m === 'login' ? 'register' : 'login'); setError('') }}>
            {mode === 'login'
      ...
      {/* ── Labels de zona (decorativos) ── */}
      <div className="ls-zone ls-zone--left">
        <span>DISTRITO</span>
        <strong>SKÁLHOLM</strong>
      </div>
      <div className="ls-zone ls-zone--right">
        <span>SETOR</span>
